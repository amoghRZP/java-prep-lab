...
public class HouseRobber {
    ...
    public int topDown(int[] nums) {
        ...
        return maxAmountRecursive(0, nums, dp);
    }

    public int maxAmountRecursive(int i, int[] nums, int[] dp) {
        if (i >= nums.length) {
            return 0;
        }
        if (dp[i] != -1) {
        ...
        return dp[i] = Math.max(maxAmountRecursive(i + 1, nums, dp), nums[i] + maxAmountRecursive(i + 2, nums, dp));
    }
